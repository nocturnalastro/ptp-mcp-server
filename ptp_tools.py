    ...
    async def get_ptp_logs(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with kubeconfig_from_base64(kubeconfig) as kubeconfig_path:
                ...
                gm_info = self.log_parser.extract_grandmaster_info(filtered)
                sync_status = self.log_parser.extract_sync_status(filtered)
                clock_hierarchy = self.log_parser.extract_clock_hierarchy(filtered)
                ...
                return {
                    ...
                    "log_entries": [
                        ...
                        for log in filtered[-100:]  # Return last 100 entries for brevity
                    ]
                }
                ...
                if check_sync:
                    try:
                        ...
                        sync_check = {
                            ...
                            "last_offsets": self._extract_per_component_offsets(filtered),
                            "last_servo_states": sync_status.get("last_servo_states")
                            or self._extract_per_component_servo_states(filtered),
                        }
